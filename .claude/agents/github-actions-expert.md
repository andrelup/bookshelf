...
- Real scripts: `dev`, `build` (`tsc --noEmit && vite build`), `preview`, `lint` (`eslint src --max-warnings=0`), `lint:fix`, `format` (`prettier --write .`), `format:check` (`prettier --check .`), `typecheck` (`tsc --noEmit`), `test` (`vitest run`), `test:watch`
...
**Documented but NOT YET IN THE TREE** — do not reference these in a workflow until they exist, but they are on the roadmap, so do not propose removing them from the docs either:
- `frontend/Dockerfile` (only `backend/Dockerfile` exists)
- Playwright / any E2E suite (`test-e2e` is vapour)
- Prometheus and Grafana — #27 (stack + dashboards) and #28 (health checks and alerts). SonarQube — #25. `infra/docker-compose.yml` has only `postgres` + `backend` today

**Already in the tree** (do not describe these as missing):
- Frontend pre-commit hooks: `.pre-commit-config.yaml` runs `eslint` and `prettier --check` as `repo: local` / `language: system` hooks. CI must stay in sync with them
- `scripts/setup.py` behind `make setup`: the one-command local bootstrap. It is plain stdlib and lives outside `backend/`, so the `ruff`/`mypy` hooks (filtered on `^backend/`) do NOT cover it
- `.gitattributes` pins `eol=lf`; a workflow must not reintroduce CRLF
...
### SonarQube is part of this project

Root `CLAUDE.md` and `README.md` state that **SonarQube is the quality gate on PRs**, and issue #25 tracks wiring it up. That is a deliberate decision of this repo — treat it as a requirement, not as stale documentation.

What that means in practice:

- The Sonar scan is one more job in the pipeline, gated behind `lint` and the test jobs, and it consumes the coverage reports they produce (`coverage.xml` for pytest, `lcov.info` for vitest). Do not make it recompute coverage
- Sonar is the gate, **not a replacement for the local checks**. `ruff`, `mypy --strict`, `eslint` and `prettier --check` still run in CI: they are what `.pre-commit-config.yaml` runs on every commit, and CI must agree with the hooks
- Credentials (`SONAR_TOKEN`, `SONAR_HOST_URL`) come from `secrets.*` only, and a fork PR has no access to them — say so explicitly and handle it rather than letting the job fail opaquely
- There is **no SonarQube service in `infra/docker-compose.yml` yet** — #25 adds it, for local use. A workflow must never assume a self-hosted instance is reachable from a GitHub runner

The free GitHub-native tooling below **complements** Sonar, it does not compete with it.
...
- [ ] Sonar credentials read from `secrets.*`, and the fork-PR case handled explicitly
